...
// --- START: CUSTOM EMBED COMPONENT FOR GITHUB DARK MODE ---
function CustomEmbed({ block, blockId }: { block: any; blockId: string }) {
  const { recordMap, components } = useNotionContext()
  // The original Embed component from react-notion-x is accessed via the context
  ...
  if (url && url.includes('gist.github.com')) {
    ...
    // The embed link used by Notion often already has the .js suffix
    ...
    return (
      <div 
        ...
        // CRITICAL FIX: Inject the data-theme attribute for dark mode detection
        data-theme='dark' 
        style={{ width: '100%' }}
      >
        {/* Render the script tag required for Gists */}
        <script src={`${gistUrl}.js`} />
      </div>
    )
  }

  // 2. Fallback to the default Embed component for all other types (YouTube, Loom, etc.)
  return <Embed block={block} blockId={blockId} />
}
// --- END: CUSTOM EMBED COMPONENT FOR GITHUB DARK MODE ---
...
      propertyTextValue,
      propertyDateValue,
      Embed: CustomEmbed // <-- INJECTING THE CUSTOM EMBED COMPONENT HERE
    }),
    []
  )
